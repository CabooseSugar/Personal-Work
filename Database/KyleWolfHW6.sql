...
(SELECT name, course_id, sec_id, semester FROM
takes NATURAL LEFT OUTER JOIN student)
NATURAL RIGHT OUTER JOIN
(SELECT course_id, sec_id, semester FROM
teaches NATURAL LEFT OUTER JOIN instructor
WHERE dept_name = 'Computer Science'))); 


-- PROBLEM 2
CREATE VIEW v AS
SELECT dept_name, name, COUNT(*) as classes FROM teaches NATURAL JOIN instructor
GROUP BY dept_name, name
UNION
SELECT dept_name, name, 0 AS classes
FROM instructor NATURAL LEFT OUTER JOIN teaches
WHERE course_id IS NULL;

SELECT dept_name, round(avg(classes), 2) AS average
FROM v
GROUP BY dept_name
ORDER BY average DESC;

SELECT dept_name, COUNT(*) AS overworked FROM v JOIN x ON (classes > average);



-- PROBLEM 3
CREATE VIEW CS101Students AS
SELECT id, name, dept_name, tot_cred, grade
FROM takes NATURAL JOIN student
WHERE course_id = 'CS-101' AND sec_id = '1' AND semester = 'Fall' AND year = 2019;
...
BEGIN
    ...
    UPDATE department
    SET dept_name = 'CS' WHERE dept_name = 'Computer Science';
...
create table takes(
	...
	foreign key (ID) references student ON DELETE SET NULL
);
